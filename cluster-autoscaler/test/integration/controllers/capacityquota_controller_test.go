...
var _ = Describe("CapacityQuota Controller", func() {
	...
	It("should correctly sum capacities of matching nodes", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(apiequality.Semantic.DeepEqual(fetchedCQ.Status.Used.Resources, cqv1alpha1.ResourceList{
	...
	It("should correctly update capacities when a new matching node is added", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(meta.IsStatusConditionTrue(fetchedCQ.Status.Conditions, capacityquota.ReconciledCondition)).To(BeTrue())
			g.Expect(fetchedCQ.Status.Used).ToNot(BeNil())
			usedCPU := fetchedCQ.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU.Equal(resource.MustParse("6"))).To(BeTrue())
		}).Should(Succeed())
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(fetchedCQ.Status.Used).ToNot(BeNil())
			usedCPU := fetchedCQ.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU.Equal(resource.MustParse("8"))).To(BeTrue())
		}).Should(Succeed())
	})
	...
	It("should correctly update capacities when a node's label is removed, causing it to stop matching", func() {
		...
		By("waiting for initial reconciliation")
		Eventually(func(g Gomega) {
			var fetchedCQ1 cqv1alpha1.CapacityQuota
			g.Expect(crClient.Get(ctx, cqKey1, &fetchedCQ1)).To(Succeed())
			g.Expect(meta.IsStatusConditionTrue(fetchedCQ1.Status.Conditions, capacityquota.ReconciledCondition)).To(BeTrue())
			g.Expect(fetchedCQ1.Status.Used).ToNot(BeNil())
			usedCPU1 := fetchedCQ1.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU1.Equal(resource.MustParse("11"))).To(BeTrue()) // 6 from initial nodes + 5 from node5

			var fetchedCQ2 cqv1alpha1.CapacityQuota
			g.Expect(crClient.Get(ctx, cqKey2, &fetchedCQ2)).To(Succeed())
			g.Expect(meta.IsStatusConditionTrue(fetchedCQ2.Status.Conditions, capacityquota.ReconciledCondition)).To(BeTrue())
			g.Expect(fetchedCQ2.Status.Used).ToNot(BeNil())
			usedCPU2 := fetchedCQ2.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU2.Equal(resource.MustParse("5"))).To(BeTrue()) // 5 from node5
		}).Should(Succeed())
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(fetchedCQ1.Status.Used).ToNot(BeNil())
			usedCPU1 := fetchedCQ1.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU1.Equal(resource.MustParse("6"))).To(BeTrue())

			var fetchedCQ2 cqv1alpha1.CapacityQuota
			g.Expect(crClient.Get(ctx, cqKey2, &fetchedCQ2)).To(Succeed())
			g.Expect(fetchedCQ2.Status.Used).ToNot(BeNil())
			usedCPU2 := fetchedCQ2.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU2.Equal(resource.MustParse("5"))).To(BeTrue())
		}).Should(Succeed())
	})
	...
	It("should exclude nodes matching the configured NodeFilter", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(fetchedCQ.Status.Used).ToNot(BeNil())
			usedCPU := fetchedCQ.Status.Used.Resources[cqv1alpha1.ResourceCPU]
			g.Expect(usedCPU.Equal(resource.MustParse("6"))).To(BeTrue()) // Only 2 + 4 from initial nodes, ignoring 1000
		}).Should(Succeed())
	})
